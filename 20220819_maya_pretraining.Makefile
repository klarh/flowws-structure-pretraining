...
STATIC_TRAIN:=flowws_keras_experimental.Train --epochs ${EPOCHS} --validation-split .3 --batch-size 4 --use-multiprocessing False --catch-keyboard-interrupt 1 --reduce-lr 1 --early-stopping 2 --early-stopping-best 1 --recompile 1 --accumulate-gradients 8 --seed ${SEED} ${EXTRA_TRAIN_ARGS}
...
for i, ref_scene in tqdm(enumerate(reversed(scenes)), total=n_frames):
    ...
    scene.save(target)
endef
export MOVIE_PYSCRIPT
MOVIE_SCRIPT := ${PYTHON} -c "$$MOVIE_PYSCRIPT"

distance_colored.%.mp4: dump.pyriodic_distance_embedding.%.sqlite.zip ${MOVIE_FRAME_DIR}
	rm -rf "${MOVIE_FRAME_DIR}"
	mkdir -p "${MOVIE_FRAME_DIR}"
	$(MOVIE_SCRIPT) "$<" ${MOVIE_OP_THRESH}
	ffmpeg -y -framerate ${MOVIE_FRAMERATE} -i "${MOVIE_FRAME_DIR}/frame.%05d.png" -filter:v "crop=(trunc(iw/16)*16):(trunc(ih/16)*16)" -c:v libx264 -preset slow -crf 20 -threads 4 -pix_fmt yuv420p "$@"

distance_colored.cP1_Po.shift_id.mp4:
	$(MAKE) -f "$(THIS_FILE)" distance_colored.cP1_Po.dump.shift_identification.pyriodic_all_frames.mp4 MOVIE_OP_THRESH=.15 FRAME_SKIP=2 MOVIE_FRAMERATE=30
	ln distance_colored.cP1_Po.dump.shift_identification.pyriodic_all_frames.mp4 "$@"

distance_colored.cI2_W.shift_id.mp4:
	$(MAKE) -f "$(THIS_FILE)" distance_colored.cI2_W.dump.shift_identification.pyriodic_all_frames.mp4 MOVIE_OP_THRESH=.18
	ln distance_colored.cI2_W.dump.shift_identification.pyriodic_all_frames.mp4 "$@"

distance_colored.iQc.shift_id.mp4:
	$(MAKE) -f "$(THIS_FILE)" distance_colored.iQc.dump.shift_identification.pyriodic_all_frames.mp4 MOVIE_OP_THRESH=.025 FRAME_SKIP=8
	ln distance_colored.iQc.dump.shift_identification.pyriodic_all_frames.mp4 "$@"
...
all_nearest_bond_pyriodic_leave_one_out: $(foreach struc,${PYRIODIC_ALL_STRUCTURES},dump.nearest_bond.pyriodic_leave_one_out.${struc}.sqlite)
all_autoencoder_pyriodic_leave_one_out: $(foreach struc,${PYRIODIC_ALL_STRUCTURES},dump.autoencoder.pyriodic_leave_one_out.${struc}.sqlite)
...
all_shift_identification_pyriodic_leave_one_out: $(foreach struc,${PYRIODIC_ALL_STRUCTURES},dump.shift_identification.pyriodic_leave_one_out.${struc}.sqlite)
all_frame_classification_pyriodic_leave_one_out: $(foreach struc,${PYRIODIC_ALL_STRUCTURES},dump.frame_classification.pyriodic_leave_one_out.${struc}.sqlite)
...
dump.nearest_bond.pyriodic_leave_one_out.%.sqlite:
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		$(patsubst $(@:dump.nearest_bond.pyriodic_leave_one_out.%.sqlite=%),,${PYRIODIC_ALL_FRAMES}) \
		${NEIGHBOR_CALC} \
		${NEAREST_BOND} ${NEAREST_BOND_ARCH} ${NEAREST_BOND_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f nearest_bond pyriodic_leave_one_out $(@:dump.nearest_bond.pyriodic_leave_one_out.%.sqlite=%)

dump.autoencoder.pyriodic_leave_one_out.%.sqlite:
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		$(patsubst $(@:dump.autoencoder.pyriodic_leave_one_out.%.sqlite=%),,${PYRIODIC_ALL_FRAMES}) \
		${NEIGHBOR_CALC} \
		${AUTOENCODER} ${AUTOENCODER_ARCH} ${AUTOENCODER_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f autoencoder pyriodic_leave_one_out $(@:dump.autoencoder.pyriodic_leave_one_out.%.sqlite=%)

dump.denoising.pyriodic_leave_one_out.%.sqlite:
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		$(patsubst $(@:dump.denoising.pyriodic_leave_one_out.%.sqlite=%),,${PYRIODIC_ALL_FRAMES}) \
		${NEIGHBOR_CALC} \
		${DENOISING} ${DENOISING_ARCH} ${DENOISING_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f denoising pyriodic_leave_one_out $(@:dump.denoising.pyriodic_leave_one_out.%.sqlite=%)

dump.shift_identification.pyriodic_leave_one_out.%.sqlite:
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		$(patsubst $(@:dump.shift_identification.pyriodic_leave_one_out.%.sqlite=%),,${PYRIODIC_ALL_FRAMES}) \
		${NEIGHBOR_CALC} \
		${SHIFT_ID} ${SHIFT_ID_ARCH} ${SHIFT_ID_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f shift_identification pyriodic_leave_one_out $(@:dump.shift_identification.pyriodic_leave_one_out.%.sqlite=%)

dump.frame_classification.pyriodic_leave_one_out.%.sqlite:
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		$(patsubst $(@:dump.frame_classification.pyriodic_leave_one_out.%.sqlite=%),,${PYRIODIC_ALL_FRAMES}) \
		${NEIGHBOR_CALC} \
		${FRAME_CLASSIFICATION} ${FRAME_CLASSIFICATION_ARCH} ${FRAME_CLASSIFICATION_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f frame_classification pyriodic_leave_one_out $(@:dump.frame_classification.pyriodic_leave_one_out.%.sqlite=%)

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.noisy_bond.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${NOISY_BOND} --subsample ${TRANSFER_SUBSAMPLE} ${NOISY_BOND_ARCH} --transfer-freeze 0 ${NOISY_BOND_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic noisy_bond "$<" \

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.nearest_bond.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${NEAREST_BOND} --subsample ${TRANSFER_SUBSAMPLE} ${NEAREST_BOND_ARCH} --transfer-freeze 0 ${NEAREST_BOND_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic nearest_bond "$<" \

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.autoencoder.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${AUTOENCODER} --subsample ${TRANSFER_SUBSAMPLE} ${AUTOENCODER_ARCH} --transfer-freeze 0 ${AUTOENCODER_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic autoencoder "$<" \

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.denoising.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${DENOISING} --subsample ${TRANSFER_SUBSAMPLE} ${DENOISING_ARCH} --transfer-freeze 0 ${DENOISING_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic denoising "$<" \

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.shift_identification.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${SHIFT_ID} --subsample ${TRANSFER_SUBSAMPLE} ${SHIFT_ID_ARCH} --transfer-freeze 0 ${SHIFT_ID_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic shift_identification "$<" \

dump.xfer.nofreeze.${TRANSFER_SUBSAMPLE}.pyriodic.frame_classification.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${FRAME_CLASSIFICATION} --subsample ${TRANSFER_SUBSAMPLE} ${FRAME_CLASSIFICATION_ARCH} --transfer-freeze 0 ${FRAME_CLASSIFICATION_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer nofreeze ${TRANSFER_SUBSAMPLE} pyriodic frame_classification "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.noisy_bond.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${NOISY_BOND} --subsample ${TRANSFER_SUBSAMPLE} ${NOISY_BOND_ARCH} --transfer-freeze 1 ${NOISY_BOND_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic noisy_bond "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.nearest_bond.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${NEAREST_BOND} --subsample ${TRANSFER_SUBSAMPLE} ${NEAREST_BOND_ARCH} --transfer-freeze 1 ${NEAREST_BOND_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic nearest_bond "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.autoencoder.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${AUTOENCODER} --subsample ${TRANSFER_SUBSAMPLE} ${AUTOENCODER_ARCH} --transfer-freeze 1 ${AUTOENCODER_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic autoencoder "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.denoising.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${DENOISING} --subsample ${TRANSFER_SUBSAMPLE} ${DENOISING_ARCH} --transfer-freeze 1 ${DENOISING_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic denoising "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.shift_identification.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${SHIFT_ID} --subsample ${TRANSFER_SUBSAMPLE} ${SHIFT_ID_ARCH} --transfer-freeze 1 ${SHIFT_ID_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic shift_identification "$<" \

dump.xfer.freeze.${TRANSFER_SUBSAMPLE}.pyriodic.frame_classification.%.sqlite: %
	${PYTHON} -m flowws.run \
		InitializeTF ${INITIALIZETF_EXTRA_ARGS} \
		${PYRIODIC_ALL_FRAMES} \
		LoadModel -f "$<" \
		ClearMetrics \
		${FRAME_CLASSIFICATION} --subsample ${TRANSFER_SUBSAMPLE} ${FRAME_CLASSIFICATION_ARCH} --transfer-freeze 1 ${FRAME_CLASSIFICATION_TRAIN} \
		flowws_keras_experimental.Save --save-model 1 -f xfer freeze ${TRANSFER_SUBSAMPLE} pyriodic frame_classification "$<" \
